...
class StartupApiTests(APITestCase):
    ...
    def setUp(self):
        self.startup_user = User.objects.create_user(
            username="teststartup", email="startup@test.com", password="123456"
        )

        self.investor_user = User.objects.create_user(
            username="investor", email="investor@test.com", password="123456"
        )
    ...
    def test_followers_count_when_saved(self):
        investor_profile = InvestorProfile.objects.create(
            user=self.investor_user, company_name="Test Investor Corp"
        )
        SavedStartup.objects.create(
            startup_profile=self.startup, investor_profile=investor_profile
        )
        response = self.client.get("/api/startups/test-startup/")
        self.assertEqual(response.status_code, 200)
