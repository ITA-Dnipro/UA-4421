...
class ServiceLayerTests(TestCase):
    ...
    def test_create_message(self):
        """Test create_message: validates participants, updates last_message_at, persists to Mongo."""
        import uuid
    ...
    def test_is_participant(self):
        """Test is_participant returns correct result."""
        conversation_id = self.chat_service.create_conversation(
            participants=[self.user1.id, self.user2.id]
        )

        self.assertTrue(self.chat_service.is_participant(conversation_id, self.user1.id))
        self.assertTrue(self.chat_service.is_participant(conversation_id, self.user2.id))
        self.assertFalse(self.chat_service.is_participant(conversation_id, self.user3.id))
        self.assertFalse(self.chat_service.is_participant('non-existent-id', self.user1.id))

    def test_is_message_participant(self):
        """Test is_message_participant returns correct result."""
        conversation_id = self.chat_service.create_conversation(
            participants=[self.user1.id, self.user2.id]
        )
        ...
        self.assertTrue(self.chat_service.is_message_participant(message_id, self.user1.id))
        self.assertTrue(self.chat_service.is_message_participant(message_id, self.user2.id))
        self.assertFalse(self.chat_service.is_message_participant(message_id, self.user3.id))
        self.assertFalse(self.chat_service.is_message_participant('invalid-id', self.user1.id))